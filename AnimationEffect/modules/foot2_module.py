...
def ani_effect(y,x,fr,effect):
    ...
    ret, mask = cv2.threshold(effect_gray, 70,255, cv2.THRESH_BINARY)
...
def foot2_effect (cap, frame, back_cap, back_frame, out, in_video, i) :
    ...
    while(cap.isOpened()):
        ...
        for j in range(len(ani_start)):
            if start <= i < start+n :
                eff = cv2.imread('./Effects/Foot2/foot2'+str(i-start).zfill(4)+'.jpg')
